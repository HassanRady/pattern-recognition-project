...
def select_features(train_df, test_df, save_path, correlation_threshold):
    selected_features = univariate_feature_selection(
        df=train_df.fillna(train_df.mean()).drop(
            columns=[utils.constants.SII_COLUMN_NAME]
        ),
        target=train_df[utils.constants.SII_COLUMN_NAME],
...
def subset_of_features(df: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    ...
    if utils.constants.SII_COLUMN_NAME in df.columns:
        features.append(utils.constants.SII_COLUMN_NAME)
    if utils.constants.PCIAT_TOTAL_CULUMN_NAME in df.columns:
        features.append(utils.constants.PCIAT_TOTAL_CULUMN_NAME)
    return df[features]
